...
func (server *Server) getAccount(ctx *gin.Context) {
	...
	if account.Owner != authPayload.Username {
		ctx.JSON(http.StatusUnauthorized, errorResponse(errors.New("account does not belong to this user")))
		return
	}
...
func (server *Server) listAccounts(ctx *gin.Context) {
	...
	arg := db.ListAccountsByUserParams{
		Owner:  authPayload.Username,
		Limit:  req.PageSize,
		Offset: (req.PageID - 1) * req.PageSize,
	}

	accounts, err := server.store.ListAccountsByUser(ctx, arg)
...
type updateAccountJSONRequest struct {
	Balance int64 `json:"balance" binding:"numeric,min=0"`
}
